...
def get_number_of_req_swaps(circuit, dist, mapping):
    ...
    for gate in circuit:
        if gate[0].name == 'cx':
            control = gate[1][0]._index
            target = gate[1][1]._index
...
def reorder_circuit_with_swap(circuit, mapping, dist, next, qubits=None):
    ...
    for t, gate in enumerate(circuit):
        if gate[0].name == 'cx':
            ...
            qubit = gate[1][0]._index
            clbit = gate[2][0]._index
            new_circuit.measure(mapping[qubit], mapping[clbit])
        else:
            new_circuit.append(gate.operation, [mapping[q._index] for q in gate[1]])
